...
export default function BookGuestHousePage() {
  ...
  const availability: DateAvailabilityMap = useMemo(() => {
    ...
    for (let d = new Date(start); d < end; d.setDate(d.getDate() + 1)) {
      ...
      map[key] = getGuestHouseDateAvailability(approved, key);
    }
    return map;
  }, [approved]);
  ...
  const handleSubmit = async (data: BookingRequestInput) => {
    const res = await fetch("/api/bookings", {
      ...
      body: JSON.stringify({ ...data, slot: "FULL_DAY" }),
    });
    if (!res.ok) {
      ...
      <main className="min-h-screen p-6">
        <div className="max-w-lg mx-auto rounded-xl bg-white border border-slate-200 p-8 shadow-sm text-center">
          ...
          <p className="mt-2 text-slate-600">
            Your Guest House booking request has been submitted. You will be notified after admin approval.
          </p>
          <Link href="/book/guest-house" className="btn-primary inline-block mt-6">
            Book Another
          </Link>
          <Link href="/" className="block mt-3 text-royal underline">Back to Home</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen p-4 md:p-6">
      ...
      <h1 className="text-2xl font-bold text-royal mb-2">Book Guest House</h1>
      <p className="text-slate-600 mb-2">
        Full Day only. Green = available, Red = booked.
      </p>
      <div className="flex flex-wrap gap-4 mb-6 text-sm">
        ...
        <span className="flex items-center gap-2">
          <span className="w-4 h-4 rounded bg-red-100" /> Booked
        </span>
      </div>
